...
**Local Lighthouse mobile /en after fixes: Perf 97 · A11y 100 · BP 100 · SEO 100** (LCP 2.3s, CLS 0, TBT 0). 149/149 unit GREEN; QA driver ALL PASS (7 sections, nav 6 links, theme flip, CV dropdown Escape=24-05, filter dims 4, game entry, /es Spanish, mobile) with 0 hydration errors. **NOTE:** this is a local proxy — the OFFICIAL DEPLOY-04 gate still needs a real Vercel deploy on `/`, `/en`, `/es`. **Residual (non-blocking):** `label-content-name-mismatch` audit (weight 0, A11y still 100) — an Experience compact-row button's visible text ≠ its expand/collapse aria-label; `astro dev` still throws `_jsxDEV is not a function` (dev JSX-runtime config, prod build unaffected).
