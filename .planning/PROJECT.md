...
**Local-done, pending operator UAT:** v3.11 Mario-World Experience Map (Phases 21–24 closed 2026-06-11) — Mario-Bros-inspired 2D world-map landing replaces the 3D skill-constellation. Worlds = companies (levels = roles); section/secret worlds for non-experience content. 5 era×stack biomes (Pradera 2007–2012 Java/JEE → Castillo 2025–2026 Claude/AI). Pixel-art Carlos avatar walks via arrow keys; drag pans camera; clicking a world plays a 600ms cinematic zoom into a `role=dialog` overlay with focus trap; Esc closes via 400ms zoom-out. Terminal-style typed commands (anywhere on page, 2s idle buffer, Backspace/Esc clear, editable-target gate) unlock hidden worlds via dynamic `SECRET_WORLDS` catalog. Hero gate at first paint routes Game vs Dev (classic CV scroll). Adaptive renderer: WebGL three.js (`PerspectiveCamera` + Sprites + biome planes + rAF tick lerping avatar/camera/zoom; lazy chunk `WebGLWorldMap-*.js`) on capable desktop; `SvgWorldMap` DOM fallback (region-per-biome, roving tabindex, accessible buttons) for SVG tier / RM users / Suspense boundary / `webglcontextlost` recovery. Click-vs-drag arbitration via `useClickVsDrag` (relocated from `src/game/` to `src/marioWorld/hooks/`). 33 commits on branch `v3.11-mario-world`. 150/150 tests GREEN (was 379 pre-Phase-24-cleanup; ~229 legacy constellation tests removed alongside their impl). Bundle gate exit 0: `index-*.js` = 66.49 kB gz (re-baselined ceiling: 90 kB HARD / 50 kB WARN; old GameMode 38.82 kB ceiling absorbed because main bundle now carries App + MarioWorld orchestrator + DevView + HeroGameGate + SvgWorldMap + overlay + i18n); `WebGLWorldMap-*.js` = 116.50 kB gz (< 125 SOFT / < 130 HARD). Lighthouse mobile HARD gate **cleared programmatically**: Perf 0.97 / A11y 1.00 / BP 1.00 / SEO 1.00 / Target-size 1. Awaiting operator real-browser 12-item UAT sweep ([`phases/v3.11-mario-world-UAT.md`](phases/v3.11-mario-world-UAT.md)) before tag `v3.11.0` + push + merge to `main`. Sprite art ships as solid-color placeholders (sharp-generated); high-fidelity art tracked under [`seeds/SEED-MARIO-WORLD-ART.md`](seeds/SEED-MARIO-WORLD-ART.md) (OPEN-ART-1). `SECRET_WORLDS` catalog ships empty (operator adds entries when narratives written). Plan canonical at [`docs/superpowers/plans/2026-06-10-mario-world-experience-map.md`](../docs/superpowers/plans/2026-06-10-mario-world-experience-map.md); spec at [`docs/superpowers/specs/2026-06-10-mario-world-experience-map-design.md`](../docs/superpowers/specs/2026-06-10-mario-world-experience-map-design.md).
...
(v3.11 Mario-World awaiting operator UAT + tag + push — see "Current State". v3.12 scope TBD after v3.11 lands on main. Carried candidates: DEPLOY-01/02/03 (v3.7), VIS-05 claude-kanban + caveman cards (v3.6 backlog), DIAGRAMS-01 cross-repo diagrams (v3.6 backlog), real `SECRET_WORLDS` entries + sprite art workstream.)
...
---
*Last updated: 2026-06-11 — milestone v3.11 Mario-World Experience Map LOCAL-DONE (Phases 21–24, 33 commits on branch `v3.11-mario-world`, 150/150 tests GREEN, bundle gate exit 0, Lighthouse mobile HARD gate cleared programmatically — Perf 0.97 / A11y 1.00 / BP 1.00 / SEO 1.00). Awaiting operator 12-item real-browser UAT sweep before tag `v3.11.0` + push + merge to `main`. v3.10 3D Constellation tag candidate still queued (operator UAT 12/12 PASSED 2026-06-10; tag not yet cut). v3.7 deploy work (Plan 11-05, DEPLOY-02/03), VIS-05, DIAGRAMS-01 carried as deferred — candidates for v3.12.*
