# Requirements Archive: v3.5 Themes, Projects & Production

**Archived:** 2026-05-12
**Status:** ⚠ PARTIALLY SHIPPED — Themes + Projects delivered; DEPLOY-01/02/03 deferred to v3.6

## Outcome

| Category | Delivered | Deferred to v3.6 |
|----------|-----------|------------------|
| Visual (VIS-01, VIS-03) | ✓ both shipped | — |
| Tech Debt (DEBT-01/02/03) | ✓ all three cleared | — |
| Deployment (DEPLOY-01/02/03) | — | ✗ all three deferred (Phase 7 never executed) |

5/8 v3.5 requirements satisfied. See `v3.5-MILESTONE-AUDIT.md` for full gap analysis.

For deferred requirements carried into v3.6, see `.planning/REQUIREMENTS.md` (Future Requirements section).
...
### Deployment — ⚠ ALL DEFERRED TO v3.6

- [ ] **DEPLOY-01** *(deferred → v3.6)*: Vercel production deploy — auto-deploy from main branch, vite-react preset, build command `npm run build`, output `dist/`
- [ ] **DEPLOY-02** *(deferred → v3.6)*: Custom domain andresmontoyat.co configured with DNS records pointing to Vercel; HTTPS via Vercel auto-cert
- [ ] **DEPLOY-03** *(deferred → v3.6)*: Auto-preview deploys per PR/branch on Vercel — verify preview URL renders OG card correctly
...
| DEPLOY-01 | Phase 7 (not delivered) | Deferred → v3.6 |
| DEPLOY-02 | Phase 7 (not delivered) | Deferred → v3.6 |
| DEPLOY-03 | Phase 7 (not delivered) | Deferred → v3.6 |
