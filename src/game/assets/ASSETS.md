...
- `Player/Player_Base/Player_Base_animations.png` + modular layers `Legs/OG_Pants/Pants_1_Black`,
  `Feet/Shoes_1_Brown`, `Chest/Lumberjack_Shirt/Lumberjack_Shirt_1_Red`, `Head/Hair_2/Hair_2_Brown`
  — `<layer>_<dir>_<0..5>` (walk) + `<layer>_<dir>_idle<0..1>`, generated by `AVATAR_FRAMES`
...
- The avatar is composited from the modular base + clothing layers (`AVATAR_LAYERS` in `tiles.js`:
  base → jeans → boots → shirt → hair), each layer sharing the base's cell rects. The sheets store
  3 directions (down, right, up); there is no left row — `scene2d.js`'s `drawAvatar` substitutes
  `'right'` and mirrors it with `drawFlipped()` for `player.dir === 'left'`. Walk cycles all 6
  stride cells; standing runs a 2-frame idle loop.
