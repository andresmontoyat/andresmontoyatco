...
**Status: ALL RESOLVED** — both questions below were functionally resolved during planning; inline `RESOLVED` annotations record the decision and the plan that carries it. No open items remain for Phase 24.
...
1. **Should the CV dropdown panel keep `role="menu"`/`role="menuitem"` on the converted native `<details>` version, or drop to no explicit role?**
   - **RESOLVED (CONTEXT.md D-05, implemented by Plan 24-02):** Drop `role="menu"`/`role="menuitem"` entirely — the native `<details>` disclosure widget with two plain `<a download>` links carries correct accessible semantics without extra ARIA. Plan 24-02 Task 1 implements the drop and flags it as a deliberate choice in an Astro comment; Task 2 adds a regression guard asserting neither role appears in the rendered output.
...
2. **Does the `count-up.js` `IntersectionObserver` `threshold: 0.4` need tuning for Hero's above-the-fold stats block, given it's already in the initial viewport on page load?**
   - **RESOLVED (CONTEXT.md D-04 + Plan 24-05 QA checks):** No preemptive code change — keep `threshold: 0.4` as shipped. D-04 confirms the shared count-up module is the single source of truth; any fine-tuning is a visual-QA matter, not an architectural one. Plan 24-05's cross-browser QA gate covers the "does count-up start before the entrance fade-in completes" visual check and authorizes adding a short fixed `setTimeout` offset inside `animate()` only if the QA pass observes the odd-timing symptom.
