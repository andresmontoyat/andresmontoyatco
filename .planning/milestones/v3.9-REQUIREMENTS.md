# Requirements Archive: v3.9 Game Mode Polish
...
- [x] **POLISH-01** Visitor landing on game mode sees the constellation **above the fold** without scrolling, on every standard viewport: desktop ≥1024px, tablet ~768px, mobile ~390px. Filters bar must NOT push the constellation below the fold on initial load. Constellation visible + minimum 1 visible node within the first viewport on all 3 breakpoints. — **Delivered 2026-06-08** (Phase 18: SkillFilters → fixed bottom-0 z-30; H1 compact; renderer slot flex-1; 259/259 tests GREEN)
- [x] **POLISH-02** Constellation never reads as static — perceptible ambient motion (or visually equivalent affordance) on **every render path including SVG**. Phase 17 ambient drift/glow pulse/halo brighten currently fires WebGL-only; SVG users (reduced-motion gated, save-data, <1024px viewport, no-WebGL clients) see a fully static rendering. Solution must keep `prefers-reduced-motion: reduce` users on the static path (a11y locked) but every other render path gets motion. — **Delivered 2026-06-08** (Phase 19: motion-safe:animate-svg-twinkle 4s ease-in-out infinite, deterministic per-node phase offset via sortIndex * 137ms; 261/261 tests GREEN)
...
| REQ-ID | Phase(s) | Status | Delivered |
|--------|----------|--------|-----------|
| POLISH-01 | Phase 18 | Complete | 2026-06-08 |
| POLISH-02 | Phase 19 | Complete | 2026-06-08 |

**Coverage:** 2/2 v3.9 requirements mapped + delivered. One requirement per phase (micro-milestone). No orphans.
